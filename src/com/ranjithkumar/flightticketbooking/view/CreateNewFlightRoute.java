...
public class CreateNewFlightRoute {
	public void addFlight() {
		...
		FlightTicketDatabase.getInstanse().getFlights().put(flightNumber,
				new Flight(flightNumber, seat, from, to, amount, flightName));
		FlightTicketDatabase.getInstanse().getTicketDetails().put(flightNumber, new HashMap<Integer, Costomer>());
		System.out.println("Flight Success Fully Added ");

	}

	public void defaultFlight() {
		FlightTicketDatabase.getInstanse().getFlights().put(301,
				new Flight(301, 2, "Tirunelveli", "Chennai", 10000, "Delhi Airways"));
