		...
		for (Map.Entry<Integer, HashMap<Integer, Costomer>> ticket : ticketProcess.getTicketDetails().entrySet()) {
			if (!ticket.getValue().isEmpty()) {
				for (Map.Entry<Integer, Costomer> costomers : ticket.getValue().entrySet()) {
					System.out.println("Costomer Name : " + costomers.getValue().getCostomerName() + "  PNR Id : "
							+ costomers.getKey() + "  Flight ID : " + costomers.getValue().getFlightId()
							+ "  Ticket Amount " + costomers.getValue().getAmount() + "  From :"
							+ costomers.getValue().getFrom() + "  TO : " + costomers.getValue().getTo());
				}
			} else {
